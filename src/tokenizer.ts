...
export class Tokenizer {
  ...
  static {
    ...
    for (let i = CharCode.A_LOWER; i <= 102; i++) { // 102 = 'f'
      Tokenizer.HEX_VALUES[i] = i - CharCode.A_LOWER + 10;
    }
    // Letters A-F (uppercase)
    for (let i = CharCode.A_UPPER; i <= 70; i++) { // 70 = 'F'
    ...
    const commonTokens = [
      '(', ')', '[', ']', '{', '}', '.', ',', '+', '-', '*', '/', '=', '!=',
      '<', '>', '<=', '>=', '~', '!~', '|', 'and', 'or', 'xor', 'implies',
      'true', 'false', 'null', 'is', 'as', 'in', 'contains', 'div', 'mod',
      'where', 'select', 'all', 'exists', 'empty', 'first', 'last', 'tail',
      'skip', 'take', 'union', 'combine', 'intersect', 'exclude', '$this',
      '%resource', '%context', '%ucum', '%sct', '%loinc', '%rxnorm'
    ];
  ...
  restoreState(state: { position: number; line: number; column: number }): void {
  ...
  nextToken(): Token {
    ...
    if (ch === CharCode.QUOTE) {
      throw this.error('Double-quoted strings are not allowed in FHIRPath. Use single quotes instead.');
    }
    ...
    while (pos < len) {
      ...
      if (ch === CharCode.SLASH && pos + 1 < len) {
        ...
        if (nextCh === CharCode.ASTERISK) {
          ...
          while (pos < len - 1) { // -1 because we need to check pos+1
            const commentCh = input.charCodeAt(pos);
            ...
            if (commentCh === CharCode.ASTERISK && input.charCodeAt(pos + 1) === CharCode.SLASH) {
              pos += 2; // Skip */ at once
              column++;
              break;
            }
  ...
  private isIdentifierStart(ch: number): boolean {
    return (ch >= CharCode.A_LOWER && ch <= CharCode.Z_LOWER) ||
      ...
      if (ch === quote) {
        ...
        if (charCount <= 1024) {
          ...
          for (let i = 0; i < charCount; i += 1024) {
            value += String.fromCharCode(...chars.slice(i, Math.min(i + 1024, charCount)));
          }
        }
        ...
        switch (escaped) {
          ...
          case 117: // u (Unicode escape)
            ...
            for (let i = 0; i < 4; i++) {
              ...
              if (hexValue === -1) {
                throw this.error('Invalid hex digit in Unicode escape sequence');
              }
          ...
          default:
            ...
            throw this.error(`Invalid escape sequence: \\${String.fromCharCode(escaped)}`);
        }
      } else {
        // OPTIMIZATION: Grow array if needed
        if (charCount >= chars.length) {
  ...
  private readNumber(): Token {
    ...
    if (this.position < this.input.length && 
        (this.input.charCodeAt(this.position) === 76 || this.input.charCodeAt(this.position) === 108)) { // L or l
  ...
  private readDateTime(): Token {
    ...
    if (this.peek() === 84) { // 'T' character code
      this.advance();
      ...
      const value = this.input.slice(start + 1, this.position); // skip @ but keep T
    ...
    while (this.position < this.input.length) {
      ...
      // - Z for UTC timezone
      if (this.isDigit(ch) || ch === 45 /* - */ || ch === 58 /* : */ || 
          ch === 46 /* . */ || ch === 43 /* + */ || ch === 90 /* Z */) {
  ...
  private readIdentifier(): Token {
    ...
    while (this.position < this.input.length && this.isIdentifierPart(this.input.charCodeAt(this.position))) {
      this.advance();
    }
    ...
    switch (value) {
      ...
      case 'div':
        ...
        if (this.position < this.input.length && this.input.charCodeAt(this.position) === CharCode.DOT) {
