...
export class Parser {
  ...
  parse(input: string): ASTNode {
    ...
    if (this.current.type === TokenType.EOF) {
      const error = new ParseError(
        "Empty expression. Please provide a valid FHIRPath expression.",
        0,
        1,
        1,
      );
    ...
    if ((this.current.type as TokenType) !== TokenType.EOF) {
      throw this.error(
        `Unexpected ${this.current.value || "token"} at end of expression. The expression appears to be incomplete or has extra tokens.`,
      );
    }
  ...
  private parsePrefixExpression(): ASTNode {
    ...
    if (!parser) {
      throw this.error(
        `Unexpected ${this.current.value || "token"} in expression. Expected an operand, function, or identifier.`,
      );
    }
    return parser();
  }
  ...
  private parsePostfixExpression(left: ASTNode): ASTNode {
    switch (this.current.type) {
      ...
      case TokenType.LPAREN:
        ...
        throw this.error(
          "Invalid function call. Function calls must have the format: functionName() or functionName(arguments).",
        );
      default:
        throw this.error(
          `Unexpected postfix operator '${this.current.value}'. Valid postfix operators are: . (dot), [ (indexer), and ( (function call)`,
        );
    }
  }
  ...
  private parseFunctionCall(name: string, start: number): FunctionCallNode {
    ...
    const rparen = this.consume(
      TokenType.RPAREN,
      'Expected closing parenthesis ")" to match opening parenthesis',
    );
  ...
  private parseDotExpression(left: ASTNode): DotNode {
    ...
    if (!VALID_DOT_TOKENS.has(this.current.type)) {
      throw this.error(
        "Expected identifier after dot operator. The dot operator must be followed by a property name or function name.",
      );
    }
  ...
  private parseIndexerExpression(left: ASTNode): IndexerNode {
    ...
    const rbracket = this.consume(
      TokenType.RBRACKET,
      'Expected closing bracket "]" to match opening bracket',
    );
  ...
  private parseQualifiedIdentifier(): string {
    if (!this.check(TokenType.IDENTIFIER)) {
      throw this.error(
        "Expected identifier in qualified identifier. Format should be: namespace:identifier",
      );
    }
    ...
    while (this.check(TokenType.DOT)) {
      ...
      if (!this.check(TokenType.IDENTIFIER)) {
        throw this.error(
          "Expected identifier after dot in qualified type. Format should be: namespace.Type or namespace.Type.SubType",
        );
      }
